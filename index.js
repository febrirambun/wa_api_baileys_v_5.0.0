...
// jika tidak ada akan membuat file last_send.json 
const dataPath = './last_send.json';
...
const simpanContact = (noWa) => {
    // const timestamp = new Date().valueOf();
    const timestamp = new Date().getTime();
    const contact = { noWa, timestamp };
    const contacts = loadContacts();

    // cek ada
    const ada = contacts.find((contact) => contact.noWa === noWa);
    if(ada){
        const newContacts = contacts.filter((contact) => contact.noWa.toLowerCase() !== noWa.toLowerCase())
...
const isSendWellcome = (noWa) => {
    ...
    const ada = contacts.find((contact) => contact.noWa === noWa);
    if(ada){
        const lastTime = ada.timestamp;
        const seconds = Math.round((curTime - lastTime) / 1000);
        console.log('seconds:'+ seconds);
        // if(seconds >= 300){
        if(seconds < 300){
            console.log("Sudah belum lewat 5 menit");
            result = false;
        }
    }
	...
	sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if(type === "notify"){
            if(!messages[0].key.fromMe && !messages[0].key.participant) {
                ...
                const sendWellcome = isSendWellcome(noWa);

                if(!messages[0].key.fromMe && sendWellcome){
                    // await sock.sendMessage(noWa, {text: "Terima kasih telah menghubungi Wa center MRT Dinamiq SHE System. Kami menerapkan system untuk memudahkan pendataan dan follow up kendala sesuai urutan, mohon dapat mengisi detail kendala dan keterangannya di link ini\r\nhttps://docs.google.com/spreadsheets/d/1FJB8Gte8lqPaWGT9rRpWZvqa3GiwnPNLzeXxirOyg2E/edit#gid=0."});
                    await sock.sendMessage(noWa, {text: "Selamat datang..."});
                }

                // simpanContact(noWa, messages[0].messageTimestamp);
                simpanContact(noWa);
                ...
                // }
                // else{
                //     await sock.sendMessage(noWa, {text: "Selamat datang, silahkan klik menu dibawah ini:"});

                //     const buttons = [
                //         {buttonId: "id1", buttonText: {displayText: 'Info 1!'}, type: 1},
                //         {buttonId: "id2", buttonText: {displayText: 'Info 2!'}, type: 1},
                //         {buttonId: "id3", buttonText: {displayText: 'Bicara langsung!'}, type: 1}
                //     ]
                //     const buttonInfo = {
                ...
                //         viewOnce:true
                //     }
                //     await sock.sendMessage(noWa, buttonInfo);
                // }
            }	
